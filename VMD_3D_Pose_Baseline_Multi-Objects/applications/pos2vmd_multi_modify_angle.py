...
def positions_to_frames(pos, upper_correctqq_left_y, upper_correctqq_right_y, frame=0, xangle=0,is_upper2_body=False):
    ...
    upper_correctqq_left = QQuaternion.fromEulerAngles(QVector3D(xangle, upper_correctqq_left_y, 0))
    upper_correctqq_right = QQuaternion.fromEulerAngles(QVector3D(xangle, upper_correctqq_right_y, 0))
    # upper_correctqq_left = QQuaternion.fromEulerAngles(QVector3D(xangle+40, 20, 30))
    # upper_correctqq_right = QQuaternion.fromEulerAngles(QVector3D(xangle+40, -20, -20))
    ...
    rotation = upper_correctqq_left * orientation * initial_orientation.inverted()
    ...
    orientation = QQuaternion.fromDirection(direction, up)
    initial_orientation = QQuaternion.fromDirection(QVector3D(-1.73, -1, 0), QVector3D(1, -1.73, 0))
    rotation = upper_correctqq_right * orientation * initial_orientation.inverted()
...
def position_list_to_vmd_multi(positions_multi, vmd_file, smoothed_file, bone_csv_file, depth_file, upright_idx, center_xy_scale, center_z_scale, xangle, mdecimation, idecimation, ddecimation, alignment, is_ik, heelpos,upper_correctqq_left_y, upper_correctqq_right_y):
    writer = VmdWriter()
    ...
    for frame, positions in enumerate(positions_multi):
        positions_to_frames(positions,upper_correctqq_left_y, upper_correctqq_right_y, frame, xangle, is_upper2_body)    
...
#smoothing function of depth value
def smoothing_depth(d, dw, frame):
  if len(dw) < frame:
    dw.append(d)
    dw_sorted = sorted(dw)
    if len(dw) % 2 != 1:
...
def calc_center_z(depth_file, upright_idx, center_z_scale):
    ...
    for idx, n in enumerate(depth_indexes) :
        ...
        interval_depth = (now_depth- depth_min) / (depth_max - depth_min)
        depth_window, interval_depth = smoothing_depth(interval_depth, depth_window, frame)

        logger.debug("m: {0}, interval_depth: {1}".format(n, interval_depth))

        bone_frame_dic["センター"][n].position.setZ(interval_depth * center_z_scale)
...
def position_multi_file_to_vmd(position_file, vmd_file, smoothed_file, bone_csv_file, depth_file, upright_idx, center_xy_scale, center_z_scale, xangle, mdecimation, idecimation, ddecimation, alignment, is_ik, heelpos, upper_correctqq_left_y, upper_correctqq_right_y):
    positions_multi = read_positions_multi(position_file)
    position_list_to_vmd_multi(positions_multi, vmd_file, smoothed_file, bone_csv_file, depth_file, upright_idx, center_xy_scale, center_z_scale, xangle, mdecimation, idecimation, ddecimation, alignment, is_ik, heelpos, upper_correctqq_left_y, upper_correctqq_right_y)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-t1', '--target1', dest='target1', type=str,
                        help='target directory1')
    parser.add_argument('-t2', '--target2', dest='target2', type=str,
                        help='target directory2')
    parser.add_argument('-t3', '--target3', dest='target3', type=str,
                        help='target directory3')
    ...
    parser.add_argument('-cl', '--correct left', dest='upper_correctqq_left_y', type=float,
                        default=0,
                        help='upper correctqq left')
    parser.add_argument('-cr', '--correct right', dest='upper_correctqq_right_y', type=float,
                        default=0,
                        help='upper correctqq right')
    args = parser.parse_args()

    # resultディレクトリだけ指定させる
    #base_dir = args.target
    base_dir = args.target1
    base_dir2 = args.target2
    base_dir3 = args.target3
    ...
    depth_file = base_dir2 + "/depth.txt"
    suffix = ""
    video_name = "video"
    '''
    ...
    #vmd_file = "{0}/output_{1:%Y%m%d_%H%M%S}{2}.vmd".format(base_dir3, datetime.datetime.now(), suffix)
    vmd_file = "{0}/output_{1}{2}.vmd".format(base_dir3, video_name, suffix)
    ...
    position_multi_file_to_vmd(position_file, vmd_file, smoothed_file, args.bone, depth_file, args.upright - 1, args.centerxy, args.centerz, args.xangle, args.mdecimation, args.idecimation, args.ddecimation, is_alignment, is_ik, args.heelpos, args.upper_correctqq_right_y, args.upper_correctqq_left_y)
