...
def predict_video(model_path, video_path, depth_path, interval, smoothed_2d):
    logger.info("深度推定出力開始")

    # 深度用サブディレクトリ
    subdir = depth_path+'/depth'
    ...
    #関節位置情報ファイル
    depthf = open(depth_path +'/depth.txt', 'w')
    ...
    logger.info("creating Gif {0}/movie_depth.gif, please Wait!".format(depth_path))
    imageio.mimsave('{0}/movie_depth.gif'.format(depth_path), png_lib, fps=60)
...
def main():
    ...
    parser.add_argument('--depth_path', dest='depth_path', help='depth path', type=str)
    args = parser.parse_args()

    logger.setLevel(level[args.verbose])

    # 関節二次元データを取得
    smoothed_2d = load_smoothed_2d(args.baseline_path)

    # 間隔は1以上の整数
    interval = args.interval if args.interval > 0 else 1

    # Predict the image
    predict_video(args.model_path, args.video_path, args.depth_path, interval, smoothed_2d)
